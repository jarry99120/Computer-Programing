...
# 初始化單一全域遊戲狀態
gs = GameState()
game_initialized = {'status': False} # 用 dict 方便在不同模組間共享狀態修正
...
def serialize_game_state():
    ...
    for i in range(gs.num_players):
        ...
        players_data.append({
            "player_id": p.player_id + 1, "score": p.score, "hand_count": p.hand_count,
            "active_suns": sorted(active_suns), "used_suns": sorted(used_suns)
        })
    ...
    return {
        "num_players": gs.num_players, "current_player": gs.current_player, "current_epoch": gs.current_epoch,
        "sun_boat_position": gs.sun_boat_position, "deck_size": gs.deck_size, "center_sun": gs.center_sun,
        "auction_count": gs.auction_count, "auction_track": auction_track, "auction_active": gs.auction_active,
        "current_bidder": gs.current_bidder, "game_over": gs.game_over, "players": players_data
    }