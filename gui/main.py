...
class Player(ctypes.Structure):
    _fields_ = [
        ...
        ("sun_used",   ctypes.c_int * 13),  # 💡 核心修正一：同步補上「籌碼是否已使用」旗標
        ("score",      ctypes.c_int),
    ]
...
class GameState(ctypes.Structure):
    _fields_ = [
        ...
        ("center_sun",        ctypes.c_int),  # 💡 核心修正二：同步補上「中央公共籌碼」
    ]
...
class AuctionDialog(tk.Toplevel):
    def __init__(self, parent, gs):
        ...
        self.current_voter = 0  # 從玩家 1 開始依序輪流明標
        ...
        # 當前出價狀況面板
        self.status_frame = tk.Frame(self, bg="#23272a", pading=10, relief="sunken", borderwidth=1)
    ...
    def ask_next_player(self):
        ...
        for i in range(self.gs.num_players):
            if i < self.current_voter:
                bid_val = self.result[i]
                txt = f"玩家 {i+1}：Pass" if bid_val == 0 else f"玩家 {i+1}：出價 [{bid_val}]"
                self.status_labels[i].config(text=txt, fg="#2ec4b6" if bid_val > 0 else "#e74c3c")
            elif i == self.current_voter:
                self.status_labels[i].config(text=f"玩家 {i+1}：正在挑選籌碼...", fg="#f1c40f")
            else:
                self.status_labels[i].config(text=f"玩家 {i+1}：等待中", fg="#99aab5")

        # 如果所有人都出完價了，關閉視窗
        if self.current_voter >= self.gs.num_players:
            self.destroy()
            return
    ...
    def cast_bid(self, val):
        # 最高出價限制限制（除了 Pass，出的價格必須大於場上目前的最高出價）
...
class RaGameGUI:
    ...
    def _refresh(self):
        ...
        # 💡 核心修正五：更新玩家面板，用「亮/暗」區分可用與已翻面的太陽籌碼
        for i in range(self.gs.num_players):
            p = self.gs.players[i]
